...
while True:
    ...
    if (len(contours) > 0):
        ...
        cv2.putText(frame, "Center: x=" + str(cx) + ", y=" + str(cy),
                    (5, 15), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0))

        cv2.putText(frame, "Distance to image center: " + 
                    str("%.2f" % sqrt((width / 2 - cx) * (width / 2 - cx) + 
                                    (height / 2 - cy) * (height / 2 - cy))), 
                                    (5, 30), cv2.FONT_HERSHEY_PLAIN,
                                    1, (0, 0, 0))
        ...
        cv2.putText(frame, "Left: " + str(leftCounter), 
                    (5, 45), cv2.FONT_HERSHEY_PLAIN, 1, 
                    (0, 0, 255) if left else (0, 0, 0))
        
        cv2.putText(frame, "Right: " + str(rightCounter),
                    (5, 60), cv2.FONT_HERSHEY_PLAIN, 1, 
                    (255, 0, 0) if not left else (0, 0, 0))

        cv2.circle(frame, (cx, cy), int(sqrt(w * w / 4 + h * h / 4)), 
                    (0, 255, 0) if centered else (0, 0, 255), 2)
        
        cv2.line(frame, (cx, 0), (cx, height), (255, 0, 0), 1)
        cv2.line(frame, (0, cy), (width, cy), (255, 0, 0), 1)

        cv2.putText(frame, "Sobj/Simg: " + 
                    str("%.2f" % (w * h / width / height * 100)) + "%", 
                    (5, 90), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0))
